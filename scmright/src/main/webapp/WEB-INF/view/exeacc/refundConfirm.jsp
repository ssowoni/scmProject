...
<script type="text/javascript">
		...
		//페이징 설정 
		var refundPageSize = 10;    	// 화면에 뿌릴 데이터 수 
		...
		$(document).ready(function(){
			...
			$('#daterange').duDatepicker({
				...
				theme: 'green', // theme 색 : red, blue, green, purple, indigo, teal, and dark 
		...
		function fSelectRefundList(currentPage){
			...
			callAjax("/exeacc/refundConfirmList.do","post","text", true, param, resultCallback);
		}
		 ...
		 function refundListResult(data, currentPage){
			...
			/* 반품지시서 클릭하면 지시서 안에서 반품지시서 내용 뿌려야함  */
				function RDocModal(retNo,supNo,coNo,strNo){
				/* 밑에 모달 폼이 만들어져 있으므로, list.jsp 같은거 또 만들 필요 없다!  */
				
						var param = { 			/* retList 에 pk가 4개다.  */
								retNo : retNo ,
								supNo : supNo,
								coNo : coNo,
								strNo : strNo
						...
						var resultCallback2 = function(data){  	  //여기서 data는 json형태 오브젝트
							...
							console.log("resultCallbackB !!!! " +  data.rDocModal.retNo);   //undefined였던것 질문함! 타고타고해야함! 벨류값이 배열이라서! 
				...
				function ResultRDoc(object) {
					...
					gfModalPop("#refundDoc"); /*  밑에있는 div를 띄우는것*/
					 ...
					 $("#retNo").val(object.retNo);
					 $("#retADate").val(object.retADate);
					 $("#prodNo").val(object.prodNo);
					 $("#prodName").val(object.prodName);
					 $("#origin_strNo").val(object.origin_strNo);  //기존창고 정보도 가지고 있음 
					 
					 $("#retCnt").val(object.refund_qty);
				...
				function makeSelectBox(object){
					...
					var targetObj = document.getElementById("pick_storage");
			        ...
			        for(var item in object){
			        	...
			        	var item_strNo = object[item].strNo;
			        	var item_strName = object[item].strName;
			        	
			        	console.log("wareName.strNo ! " + object[item].strNo);
			        	console.log("wareName.strName ! " + object[item].strName);
			        	
			            //console.log("wareName.ware_no ! " + whinfoarr[i].ware_no);
						//console.log("wareName.ware_name ! " +  whinfoarr[i].ware_name);
						
						var option = document.createElement('option');
			        	option.value = item_strNo;
						option.text = item_strName;
				...
				function  fConfirmRefund(retNo, supNo, coNo,strNo ){  /* 파라미터 4개: listjsp 에서 4개로 넘겨주니까  */
				/* 4가지 값을 가져가야 할 것 같다. retList 테이블의 프라이머리키 4개,. 근데 값을 어떻게 가져가지?? ajax타야하나? */
					...
					var param =  {			 				/* 기존창고와 새로운창고를 같이 보낸다.  json으로 보내기 위한 것. string: 객체 이다!! 이거 전역변수로 쓰면 안됨!  */
							retNo : retNo, 					/* retList 에 pk가 4개다.  */
							supNo : supNo,
							coNo : coNo,
							strNo : strNo  			/* 창고리스트를위해서 */		
					}
					...
					swal("최종 반품 승인 요청하시겠습니까?", {
						  buttons:{
							...
							switch(value){
							case "yes": 
								callAjax("/exeacc/clickCeoRefundConfirm.do","post","text", true, param, resultCallback3);
								break;
							case "no": 
								break;
					...
					<li class="contents">
						...
						<div class="content">
							...
							<select id="searchKey" name="searchKey" style="width: 150px; height: 32px;" v-model="searchKey">
									...
									<option value="com_name">반품 고객</option>
